$ -> $.get "reflection.coffee", (reflection) -> new class
	constructor: ->
		...
		CodeMirror.commands.save = => localStorage.state = @editor.getValue()
			...
			while col < @width
				...
				if ch
					if /\t/.test(ch)
						...
						token = @editor.getTokenAt(line: y, ch: col + 1).className #this line is killing performance
						switch token
	...
	resize: =>
		@windowHeight = $(window).height()
		console.log @windowHeight > @lines
